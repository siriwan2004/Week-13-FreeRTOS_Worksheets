...
|-------|-------------|-----------------|--------------|----------------|------------|
| 1 (Normal) | 14| | 13|3 |3 |92.9 |
| 2 (Multiple Give) |95 |95 |40 |5 |100 |
| 3 (Short Timeout) |18 | 18| 5|4 |100 |

### คำถามสำหรับการทดลоง
1. เมื่อ give semaphore หลายครั้งติดต่อกัน จะเกิดอะไรขึ้น?

Binary semaphore เก็บค่าได้แค่ 0/1 เท่านั้น ถ้ามันเป็น 1 อยู่แล้ว แล้ว xSemaphoreGive() อีก → สัญญาณเกินจะถูกทิ้ง (ไม่สะสม) ดังนั้น Multiple Give ไม่ทำให้ count มากกว่า 1 และอาจทำให้ “อีเวนต์ล้นหาย” ถ้า Consumer ยังไม่ทัน take

2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?

ต้องใช้ xSemaphoreGiveFromISR() เท่านั้น เพราะเป็นเวอร์ชันที่ปลอดภัยต่อบริบท ISR และรองรับ portYIELD_FROM_ISR() เพื่อสลับคอนเท็กซ์ทันทีเมื่อปลดบล็อก Task ที่มี priority สูงกว่า

3. Binary Semaphore แตกต่างจาก Queue อย่างไร?

Binary Semaphore: ใช้สำหรับ “สัญญาณเหตุการณ์ / การซิงค์” (มี/ไม่มี = 1/0) ไม่ถือข้อมูล payload และ ไม่บัฟเฟอร์หลายเหตุการณ์

Queue: เป็นบัฟเฟอร์ เก็บข้อมูลหลายรายการ (FIFO) ใส่ struct/ค่าตัวเลข/ข้อความได้ เหมาะกับการ ส่งข้อมูล มากกว่า แถมรองรับการค้างข้อมูลหลายชิ้น (length > 1)
