...
|-------|-----------|-----------|--------------|----------|---------------------|
| 1 (3R, 5P) | 3 | 5 | 100|650 |88 |
| 2 (5R, 5P) | 5 | 5 |100 |120 |62 |
| 3 (3R, 8P) | 3 | 8 |100 |1800 | 97|

### คำถามสำหรับการทดลอง
1. เมื่อ Producers มากกว่า Resources จะเกิดอะไรขึ้น?

เกิด contention สูง → ทุกคำขอต้อง ต่อคิวรอ counting semaphore

Avg Wait เพิ่มขึ้นชัดเจน 

Resource Utilization พุ่งใกล้ 100% (ทรัพยากรถูกใช้งานตลอด)

Success Rate ยัง 100% ได้ ถ้า xSemaphoreTake(..., 8000ms) ไม่หมดเวลา; แต่ถ้าช่วงไหนรอนานเกิน timeout จะเริ่มเห็น failed_acquisitions ลด Success Rate ทันที

2. Load Generator มีผลต่อ Success Rate อย่างไร?

สร้าง burst ของคำขอ → ทำให้ คิวรอแน่นชั่วคราว, Avg Wait กระโดดขึ้น

ถ้า เวลารอรวม ยัง < 8000ms (timeout ของ producers) → Success Rate ยัง 100%

ถ้า burst หนักจนบางคำขอรอ เกิน 8000ms → จะเกิด timeout → Success Rate ลดลง และนับเป็น failed_acquisitions++

3. Counting Semaphore จัดการ Resource Pool อย่างไร?

ทำหน้าที่เป็น ตัวนับสิทธิ์ (tokens) เท่ากับจำนวน resources ที่มี (MAX_RESOURCES)

แต่ละคำขอทำ xSemaphoreTake() → ลดนับ 1 เพื่อกันไม่ให้มีผู้ใช้พร้อมกันเกินจำนวนทรัพยากร

เสร็จงานแล้ว xSemaphoreGive() → คืนนับ 1 ให้ผู้อื่นใช้ต่อ
