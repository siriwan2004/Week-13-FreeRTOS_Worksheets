...
เมื่อ Queue เต็ม → xQueueSend จะบล็อกตามเวลาที่กำหนด หรือถ้าไม่ให้รอก็จะส่งไม่สำเร็จทันที

2. เมื่อ Queue ว่าง การเรียก `xQueueReceive` จะเกิดอะไรขึ้น?

เมื่อ Queue ว่าง → xQueueReceive จะรอจนมีข้อมูลหรือจน timeout หมด แล้วคืนค่าไม่สำเร็จถ้าไม่มีข้อมูลเข้ามา

3. ทำไม LED จึงกะพริบตามการส่งและรับข้อความ?

LED เป็นตัวบอกเหตุการณ์ (Event Indicator) แสดงการส่ง/รับสำเร็จของแต่ละ Task
