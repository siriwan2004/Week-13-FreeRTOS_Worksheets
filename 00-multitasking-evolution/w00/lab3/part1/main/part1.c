...
void cooperative_scheduler(void)
{
    ...
    while (1) {
        ...
        if (gpio_get_level(BUTTON_PIN) == 0 && !emergency_flag) {
            ...
            ESP_LOGW(TAG, "⚠️ Emergency button pressed!");
        }
...
// ✅ Add this main entry point for ESP-IDF
void app_main(void)
{
    // GPIO configuration
    ...
    gpio_config_t button_conf = {
        ...
        .pull_up_en = 1,  // Internal pull-up for button
        .pull_down_en = 0,
        .intr_type = GPIO_INTR_DISABLE
    };
    gpio_config(&button_conf);

    ESP_LOGI(TAG, "System initialized. Starting cooperative scheduler...");
